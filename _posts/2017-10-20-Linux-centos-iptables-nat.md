...
如下图：
![](http://oy4e0m51m.bkt.clouddn.com/1.png)
...
#### 3.地址转换表：
![](http://oy4e0m51m.bkt.clouddn.com/%E8%A1%A81.jpg)
#### 4.通过PREROUTING链控制：
##### (1)原因：
...
#### 3.地址转换表：
![](http://oy4e0m51m.bkt.clouddn.com/%E8%A1%A82.jpg)
#### 4.通过PREROUTING链控制：
##### (1)原因：
外网访问内网，真正的目标地址是内网地址，但是请求报文中目标地址是B。当请求到达B的时候，这时候是不能经过nat服务器上的路由，如果经过，由于还没有转换地址回目标内网地址，路由发现这个目标地址就是本机地址，就不会再转发给内网地址，因此要进行路由前控制。
##### (2)语法和选项：
...
#### 2.地址转换表
![](http://oy4e0m51m.bkt.clouddn.com/3.jpg)

### 实验：实现SNAT和DNAT
#### 1.实验目的
通过一台主机B做SNAT和DNAT，分别实现内网主机A访问外网主机C，外网主机C访问内网主机A，并在最后实现REDIRECT功能。
#### 2.实验图示
![](http://oy4e0m51m.bkt.clouddn.com/2.png)
#### 3.实验环境
##### (1)第一台主机A，在内网，IP地址为192.168.65.69
![](http://oy4e0m51m.bkt.clouddn.com/3.png)
##### (2)第二台主机B，有两个网卡，一个链接内网，地址为192.168.65.68；一个链接外网，地址为172.18.65.68
![](http://oy4e0m51m.bkt.clouddn.com/4.png)
##### (3)第三台主机C，在外网，地址为172.18.65.74
![](http://oy4e0m51m.bkt.clouddn.com/5.png)
#### 4.实现SNAT
##### (1)为A主机配置路由表
A主机想要访问外网，那么其网关需要指向B主机的内网地址
{% highlight javascript linenos=table %}
[root@hostA ~]# ip route add default via 192.168.65.68
[root@hostA ~]# route -n
{% endhighlight %}
![](http://oy4e0m51m.bkt.clouddn.com/6.png)
这时候主机A是可以ping通主机B的外网地址，但是无法ping通主机C
{% highlight javascript linenos=table %}
[root@hostA ~]# ping 172.18.65.68
[root@hostA ~]# ping 172.18.65.74
{% endhighlight %}
![](http://oy4e0m51m.bkt.clouddn.com/7.png)
>注：不需要为C机器配置路由。因为经过B的地址转换，私网IP转为了公网IP，而B的公网IP和C的IP同出一个网段，而且A看到的请求报文是来自B的公网IP，因此回应报文也是向B的公网IP发送。

##### (2)在主机B开启路由，以及做SNAT策略
{% highlight javascript linenos=table %}
[root@hostB ~]# echo 1 > /eproc/sys/net/ipv4/ip_forward
[root@hostB ~]# iptables -t nat -A POSTROUTING -s 192.168.0.0/16 -j SNAT --to-source 172.18.65.68
{% endhighlight %}
##### (3)在主机A上进行测验，在主机C上抓包，并测验
{% highlight javascript linenos=table %}
[root@hostA ~]# ping 172.18.65.74
{% endhighlight %}
![](http://oy4e0m51m.bkt.clouddn.com/8.png)
{% highlight javascript linenos=table %}
[root@hostC ~]# tcpdump -i ens33 -nn icmp
{% endhighlight %}
![](http://oy4e0m51m.bkt.clouddn.com/9.png)
{% highlight javascript linenos=table %}
[root@hostC ~]# ping 192.168.65.69
{% endhighlight %}
![](http://oy4e0m51m.bkt.clouddn.com/10.png)
#### 5.实现DNAT
...
{% highlight javascript linenos=table %}
[root@hostA ~]# yum -y install httpd
[root@hostA ~]# echo “you had already got this web”  > /var/www/html/index.html
[root@hostA ~]# vim /etc/httpd/conf/httpd.conf
Listen 192.168.56.69:8080
[root@hostA ~]# service httpd restart
[root@hostA ~]# lsof -i  :8080
[root@hostA ~]# ss -ntlp
{% endhighlight %}
![](http://oy4e0m51m.bkt.clouddn.com/11.png)
##### (2)在主机B设置防火墙DNAT策略
{% highlight javascript linenos=table %}
[root@hostB ~]# iptables -t nat -A PREROUTING -s 0/0 -d 172.18.65.68 -p tcp --dport 80 -j DNAT --to-destination 192.168.65.69:8080
{% endhighlight %}
![](http://oy4e0m51m.bkt.clouddn.com/12.png)
##### (3)在主机C上进行测试，并查看主机A的访问日志
{% highlight javascript linenos=table %}
[root@hostC ~]# curl 172.18.65.68:80
you had already got this web
{% endhighlight %}
![](http://oy4e0m51m.bkt.clouddn.com/13.png)
{% highlight javascript linenos=table %}
[root@hostC ~]# cat  /var/log/httpd/access_log
{% endhighlight %}
![](http://oy4e0m51m.bkt.clouddn.com/14.png)
